...
function updateButtons(addRow, rmRow, addCol, rmCol) {
    if (tbody.children.length >= MAX) addRow.disabled = true;
    else addRow.disabled = false;
    if (tbody.children.length <= MIN) rmRow.disabled = true;
    else rmRow.disabled = false;
    // const reqiredChildLength = tbody.firstChild.children.length;
    if (tbody.firstChild.children.length >= MAX) addCol.disabled = true;
    else addCol.disabled = false;
    if (tbody.firstChild.children.length <= MIN) rmCol.disabled = true;
    else rmCol.disabled = false;
}
...
removeColumn.addEventListener("click", (ev)=>{
    if (ev.target.closest("button") && tbody.firstChild.children.length > MIN) {
        ...
        const rows = Array.from(tbody.children);
        // iterating each row
        for (const row of rows)row.removeChild(row.lastElementChild);
    }
    updateButtons(appendRow, removeRow, appendColumn, removeColumn);
});
